...
class DeviceWorker(QObject):
    ...
    def _poll(self) -> None:
        ...
        self.status.emit(self.device.status())
...
class DevicePanel(QWidget):
    ...
    def __init__(self) -> None:
        ...
        job = QVBoxLayout()
        job.addLayout(state)
        job.addWidget(self.frame_button)
    ...
    def _set_busy(self, busy: bool) -> None:
        self._update_buttons(busy)
        if not busy:
            self.pause_button.setChecked(False)

    def _update_buttons(self, busy: bool) -> None:
        ready = self._connected and not busy
        self.frame_button.setEnabled(ready)
        self.engrave_button.setEnabled(ready)
        self.pause_button.setEnabled(self._connected and busy)
        self.abort_button.setEnabled(self._connected and busy)
